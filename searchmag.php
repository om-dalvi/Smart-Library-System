  ...
  <center>
         ...
         <div class="row">
          <table class="table table-striped">
            <thead>
              <tr>
                <th scope="col">Magizne_ID.</th>
                <th scope="col">Magzine_Name</th>
            ...
            <tbody>
              ...
              {
                echo $_GET['search'];
                $con_s_sql=pg_query($dbconn,"select * from magzine where concat(magzineid,name,publication,edition) like '%$_GET[search]%'");
                ...
                while($con_s_res=pg_fetch_assoc($con_s_sql))
                {
                  ...
                  echo '<tr>
                  <th scope="row">'.$n.'</th>
                  <td>'.$con_s_res['magzineid'].'</td>
                  <td>'.$con_s_res['name'].'</td>
                  <td>'.$con_s_res['publication'].'</td>
                  <td>'.$con_s_res['edition'].'</td>';
                  $n++;
                }
              }
              else{
              ?>
              <?php
                  $con_sql=pg_query($dbconn,"select * from magzine");
                    $n=1;
                    while ($con_res=pg_fetch_assoc($con_sql)) {
                      
                      echo '<tr>
                      
                      <td>'.$con_res['magzineid'].'</td>
                  <td>'.$con_res['name'].'</td>
                  <td>'.$con_res['publication'].'</td>
                  <td>'.$con_res['edition'].'</td>';
                      $n++;
                    }
                  }
                  pg_close($dbconn);
